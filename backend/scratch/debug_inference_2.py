...
async def debug_latest_failed():
    async with AsyncSessionLocal() as db:
        ...
        print(f"Downloaded {len(image_bytes)} bytes. Running background task directly...")
        try:
            # Re-running the task exactly as FastAPI does it
            ...
            # Let's check if status updated to success
            updated_det = await db.get(DiseaseDetection, detection.id)
            print(f"Final status in DB: {updated_det.status}")
